    ...
    public partial class NewParameterSync : System.Windows.Forms.Form
    {
        ...
        private void category_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (catagories.Contains(categoryComboBox.Text))
            {
                ...
                parameters = RevitUtils.GetParameters(cat);
                parameters.Sort();
        ...
        private void ShowGhostSuggestion()
        {
            ...
            if (lastOpen > lastClose)
            {
                ...
                if (currentSuggestions.Count > 0)
                {
                    ...
                    RevitUtils.GetUnit(catagories.get_Item(categoryComboBox.Text), match, out string unit, out ForgeTypeId unitID);
                    match += "]";
                    if (unit != null)
                    {
                        match += "{ " + unit + "}";
            ...
            else if (lastOpenBrac > lastCloseBrac && lastClose != -1 && lastOpen != -1)
            {
                ...
                RevitUtils.GetUnit(catagories.get_Item(categoryComboBox.Text), parameter, out string unit, out ForgeTypeId unitID, out ForgeTypeId specTypeId);
