    ...
    public partial class ExcelLinkUI : System.Windows.Forms.Form
    {
        ...
        private void Up_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(StatusTextBox.Text) && !StatusTextBox.Text.Contains("File not found."))
            {
                ...
                Excel.Update(FolderTextBox.Text + '\\' + FileTextBox.Text,
                    ScheduleTextBox.Text, workSheetTextBox.Text, AreaTextBox.Text);
                t.Commit();

                int line = linkUI.findLineIndexFromDataRow(selected);
                linkUI.removeLineFromSave(line);
                linkUI.newLink(FolderTextBox.Text + '\\' + FileTextBox.Text,
        ...
        private void RemLnk_Click(object sender, EventArgs e)
        {
            ...
            {
                ViewSchedule schedule = linkUI.getScheduleFromName(ScheduleTextBox.Text);
                if (schedule != null)
                {
                    t.Start("Remove Sheet");
                    linkUI.doc.Delete(schedule.Id);
                    t.Commit();
                }
        ...
        private void NewLnk_Click(object sender, EventArgs e)
        {
            ...
            if (editLinkForm.ShowDialog(this) == DialogResult.OK)
            {
                ...
                string scheduleName = Excel.Update(path, ScheduleTextBox.Text, name, workSheet, area);
                t.Commit();
                ...
                if (scheduleName != null)
                {
                    int line = linkUI.findLineIndexFromDataRow(selected);
                    linkUI.removeLineFromSave(line);
                    linkUI.newLink(path, workSheet, area, sheet, scheduleName);
                    InfoGrid.Rows.Clear();
                    loadSaveFile();
                }
            }
        ...
        {
            linkUI.setActiveViewSheet(ViewTextBox.Text);
        }
