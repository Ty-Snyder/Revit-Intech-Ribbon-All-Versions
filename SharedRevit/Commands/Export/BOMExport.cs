    ...
    public static class utility
    {
        public static char IndexToColumn(int a)
        {
            ...
            catch
            {
                ...
                Autodesk.Revit.UI.TaskDialog.Show("Error", "This program was coded to handle 26 rows or less. Please edit source code to fix this error");
                return 'A';
            }
                ...
                using (ExcelPackage templatePackage = new ExcelPackage(template))
                {
                    ...
                    foreach (ViewSchedule vs2 in selected)
                    {
                        using (ExcelPackage newPackage = new ExcelPackage(excelFileName))
                        {
                            ...
                            string tablerange = "A" + numOfHeaders + ":" + IndexToColumn(nColumns - 1) + (nRows + numOfHeaders); //get data range
