...
namespace SharedRevit.Commands
{
    ...
    public class TagSettings : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            var RevitUtils = SharedRevit.Utils.RevitUtilService.Get();
            RevitUtils.init(doc);

            //start settings form
            DialogResult form = System.Windows.Forms.DialogResult.None;
            using (SharedRevit.Forms.Settings.TagSetting selectionForm = new SharedRevit.Forms.Settings.TagSetting())
            {
